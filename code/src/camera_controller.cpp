...
CameraController::CameraController(PerspectiveCamera *cam, double centerDistance) {
    mpCamera = cam;
...
void CameraController::arcBallRotation(int x, int y) {
    ...
    if (mViewport[2] > mViewport[3]) {
        scale = (double) mViewport[3];
    } else {
        scale = (double) mViewport[2];
    }
    ...
    if (dotprod != 1) {
        ...
        double angle = 2.0f * std::acos(dotprod);
...
void CameraController::planeTranslation(int x, int y) {
    ...
    double d = double(mViewport[3]) / 2.0f / mTanPerspective;

    // compute up plane intersect of clickpoint (wrt fovy)
    double su = -sy + mViewport[3] / 2.0f;
    double cu = -cy + mViewport[3] / 2.0f;

    // compute right plane intersect of clickpoint (ASSUMED FOVY is 1)
    double sr = (sx - mViewport[2] / 2.0f);
    double cr = (cx - mViewport[2] / 2.0f);
