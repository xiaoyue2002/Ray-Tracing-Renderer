...
public:
    ...
    float max(){
        float larger = m_elements[0] > m_elements[1] ? m_elements[0] : m_elements[1];
        larger = m_elements[2] > larger ? m_elements[2] : larger;
        return larger;
    }
    float dot_mul(const Vector3f& b){
        return m_elements[0] * b.x() + m_elements[1] * b.y() + m_elements[2] * b.z();
    }
private:
...
Vector3f operator * ( const Vector3f& v0, const Vector3f& v1 );
Vector3f operator / ( const Vector3f& v0, const Vector3f& v1 );
Vector3f operator % (const Vector3f& v0 ,Vector3f& v1 );
